...
- **Diff** — scoped comparison against the base. `scripts/speaker-editor-core.mjs`, gateway runtime/schemas, vendor/codec files, workflows, Pages/infrastructure/configuration, audio/data and unrelated content are unchanged. `audio-processor.mjs` adds the DOM timeline import/render call and S/M button labels, and refreshes fit bounds before applying the responsive source fit action; its DSP code is unchanged.
...
The post-commit screenshot run at `2ac3ef8` passed the corrective scenarios but failed the existing strict source-fit scrollbar assertion after responsive viewport changes. The fit action could pass a previous viewport minimum into the bounds recalculation. It now refreshes bounds before choosing the fit value; the existing full-width assertion remains unchanged. That failed run is not counted as final validation.
