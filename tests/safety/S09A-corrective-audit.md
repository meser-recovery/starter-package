...
- **Diff** — scoped comparison against the base. `scripts/speaker-editor-core.mjs`, gateway runtime/schemas, vendor/codec files, workflows, Pages/infrastructure/configuration, audio/data and unrelated content are unchanged. `audio-processor.mjs` adds only the DOM timeline import/render call; its DSP code is unchanged.
