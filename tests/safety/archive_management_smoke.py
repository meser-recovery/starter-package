...
def check_archive_management(browser, base_url, screenshot_dir=None):
    ...
    try:
        ...
        # Incoming sessions with deleted sources retain the project and expose exact local recovery without canonical editing.
        details_for(page.locator('#session-list .archive-card').filter(has_text='Моё исправление'))
        open_management()
        ...
        page.wait_for_function("!document.getElementById('speaker-project-recovery').hidden && document.getElementById('speaker-project-recovery-status').textContent.includes('исходные дорожки недоступны')")
        assert page.locator('#speaker-project-recovery-attach').is_visible()
        ...
        for workflow in ('announcement', 'speaker'):
            ...
            assert page.locator('#speaker-editor').is_hidden()
        start = len(trace)
        page.goto(base_url + f"/Audio-Editor.html?session={snapshot['archived']['id']}&workflow=speaker")
        page.wait_for_function("document.getElementById('source-session-status').textContent.includes('Новая обработка недоступна')")
        readonly(start)
        page.locator('#source-session-mode-archive').click()
        shot(f"ineligible-{snapshot['archived']['id'][:3]}-390", '#source-session-archive-panel')
        start = len(trace)
        page.goto(base_url + '/Audio-Editor.html?session=bad&workflow=speaker')
        page.wait_for_function("document.getElementById('source-session-status').textContent.includes('Некорректная ссылка')")
        readonly(start)
        page.locator('#source-session-mode-archive').click()
        shot('ineligible-bad-390', '#source-session-archive-panel')
        command('purge-empty')
        start = len(trace)
