...
def check_source_session_archive(browser, base_url: str, screenshot_dir: Path | None = None) -> None:
    ...
    try:
        ...
        assert track_id in page.locator("#source-session-announcement-tracks").inner_text()
        ...
        assert session_id in summary and "archive-source.wav" in summary and "без изменения байтов" in summary
        assert "audio/wav" in summary and "предварительно версия 1" in summary
        if screenshot_dir:
            page.set_viewport_size({"width": 390, "height": 900})
