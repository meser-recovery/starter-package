...
def check_audio_processor_contract(errors: list[str]) -> None:
    ...
    if re.search(r'^import\s+(?!\{\s*(?:sha256Hex|renderSourceTimeline|createAudioTransport|createAudioMeters|createWaveformDetail)\s*\}\s+from\s+"\./(?:audio-archive-client|audio-timeline|audio-transport|audio-meters|audio-waveform-detail)\.mjs";)', source, re.M):
