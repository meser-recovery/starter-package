...
Status: implementation and guarded production activation evidence. This is not independent acceptance, merge authorization, Pages deployment, or proof of real iPhone Safari/ITP behavior.
...
| Guarded production activation / rollback | Inert package builder, two-gate activation, captured prior image identity, checksum/source guards, health/header/cookie replay checks, automatic exact-image rollback, operator checklist | Exact reviewed artifact activated; activation checks and independent public read-only checks passed; operator-script regression covers the `pipefail` layout check | Activated; operator defect corrected |
| Real iPhone/Safari gate | Requires matching Pages deployment, retained-fixture read-only check, redacted device/runtime evidence and logout | Independent public read-only checks passed, but no real-device evidence is recorded here | Intentionally unperformed |

## Guarded production activation record

- The first activation attempt stopped at the artifact-layout guard before container replacement. After the refusal, the current container identity matched the captured `prior-container-id`.
- The reviewed artifact SHA-256 was `ff5d766002b93e5307b93526bad78138b7ced6f8e95a7f77dcf0d049d92a59b3` for exact runtime source `a88fc334f929202373e34d97a6ef960cd988efbf`.
- An operational copy with the single replacement `grep -Eq` -> `grep -E` successfully activated runtime source `a88fc334f929202373e34d97a6ef960cd988efbf`.
- Rollback identity: `/var/backups/meser-audio-archive/s10a/20260918T081115Z`.
- Activation checks and independent public read-only checks passed. The corrective delta changes only the operator script, its regression test, and this evidence record, so no repeat gateway activation is required or performed.
- Raw production logs are intentionally excluded from the repository. SHA-256 only:
  - `S10A-production-gateway-activation-first.log`: `7c5cd94a8728523c1cef934c195326e3be55acde8f505476f8e438d64496b3c4`
  - `S10A-production-gateway-activation-success.log`: `88d8f510556a7accdd6dd90e7532e0504a2770251853fa40d1c8693528c21498`
